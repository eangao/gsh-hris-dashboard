...
const Religion = () => {
  ...
  // Optimize getReligions with useCallback
  const getReligions = useCallback(() => {
    const obj = {
  ...
  }, [successMessage, errorMessage, getReligions, dispatch]);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-blue-100 text-sm sm:text-base">
              Manage religious affiliations and beliefs
            </p>
          </div>
      ...
      {/* Enhanced Religion Search (Matching Cluster.jsx) */}
      <div className="bg-white shadow-sm rounded-lg p-6 mb-6 border border-gray-200">
        <Search
          setPerpage={setPerpage}
          setSearchValue={setSearchValue}
          searchValue={searchValue}
          inputPlaceholder={"Search religions by name"}
        />
        ...
        {searchValue && (
          <div className="mt-4 text-sm text-gray-600">
            {loading ? (
              ...
              <span>
                Found {totalReligion} result(s) for "{searchValue}"
              </span>
            )}
          </div>
        )}
              ...
              <tr className="bg-gradient-to-r from-gray-50 to-blue-50 border-b border-gray-200">
                <th className="p-4 text-sm font-semibold text-gray-700">
                  <div className="flex items-center">
                    <FaPray className="h-4 w-4 mr-2 text-blue-600" />
                    Religion Name
                  </div>
                </th>
                ...
                <tr>
                  <td colSpan="2" className="p-8 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      <FaPray className="h-12 w-12 text-gray-300" />
                      <p className="text-lg font-medium">No religions found</p>
                      <p className="text-sm">
                        {searchValue
                          ? `No results for "${searchValue}". Try a different search.`
                          : "Get started by adding your first religion."}
                      </p>
                    </div>
                  </td>
                </tr>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center flex-shrink-0 border border-blue-200">
                          <span className="text-blue-700 font-bold text-sm">
                            {religion.name?.charAt(0).toUpperCase()}
                          </span>
                        </div>
            ...
            <div className="flex items-center text-sm text-gray-700">
              <div className="flex items-center gap-2">
                ...
                <span>
                  ...
                  to{" "}
                  <span className="font-medium text-blue-700">
                    {Math.min(currentPage * perPage, totalReligion)}
                  </span>{" "}
                  of{" "}
                  <span className="font-medium text-blue-700">
                    {totalReligion}
            ...
            <div className="flex items-center justify-center sm:justify-end">
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600 hidden sm:block">
                  Page {currentPage} of {Math.ceil(totalReligion / perPage)}
                </span>
                <Pagination
