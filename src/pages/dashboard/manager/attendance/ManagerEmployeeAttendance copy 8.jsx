...
const ManagerEmployeeAttendance = () => {
    ...
    if (originalAttendances && originalAttendances.length > 0) {
      ...
      if (searchValue && searchValue.trim() !== "") {
        ...
        if (isEmployeeId) {
          ...
          console.log("Total attendance records:", filteredData.length);
          ...
          // Try to match by partial name or ID
          filteredData = filteredData.filter((record) => {
            if (!record) return false;

            // Match by employee name in attendance record
            const employeeNameMatch =
              record.employeeName &&
              record.employeeName.toLowerCase().includes(lowerCaseSearch);

            // Match by employee ID in attendance record
            const employeeIdMatch =
              record.hospitalEmployeeId &&
              record.hospitalEmployeeId.toString().includes(lowerCaseSearch);

            // Match against full employee data from employees array
            let matchByEmployeeObject = false;

            if (employees && employees.length > 0 && record.employeeId) {
              const matchingEmployee = employees.find(
                (emp) => emp._id === record.employeeId
              );

              if (matchingEmployee && matchingEmployee.personalInformation) {
                const { firstName, lastName, middleName } =
                  matchingEmployee.personalInformation;

                // Try various name formats
                const firstNameMatch =
                  firstName &&
                  firstName.toLowerCase().includes(lowerCaseSearch);
                const lastNameMatch =
                  lastName && lastName.toLowerCase().includes(lowerCaseSearch);
                const middleNameMatch =
                  middleName &&
                  middleName.toLowerCase().includes(lowerCaseSearch);
                ...
                const fullNameMatch = fullName.includes(lowerCaseSearch);
                const reverseNameMatch = reverseName.includes(lowerCaseSearch);

                matchByEmployeeObject =
                  firstNameMatch ||
                  lastNameMatch ||
                  middleNameMatch ||
                  fullNameMatch ||
            ...
            return (
              employeeNameMatch || employeeIdMatch || matchByEmployeeObject
            );
          });
        }
      }

      // Update filtered attendance data for display
      // Only update with filtered data if we're actively searching
      if (searchValue && searchValue.trim() !== "") {
        console.log(
          "Updating Redux with filtered data:",
          filteredData.length,
          "records"
        );
        dispatch({
          type: "attendance/setAttendances",
          payload: {
        ...
        console.log(
          "Updating Redux with original data:",
          originalAttendances.length,
          "records"
        );
        // If not searching, use the original data
        dispatch({
          type: "attendance/setAttendances",
          payload: {
            attendances: originalAttendances,
            totalAttendance: originalAttendances.length,
          },
        });
      }
  ...
  }, [searchValue, originalAttendances, dispatch, employees]); // Handle department change
        ...
        <div className="bg-white shadow-sm rounded-lg p-6 mb-6 border border-gray-200">
          <EmployeeSearch
            ...
            setSearchValue={(value) => {
              // If value is an employee ID (when selecting from dropdown), use it directly
              // Otherwise, it's a text search value
              setSearchValue(value);
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-blue-600">
                  {totalAttendance} record{totalAttendance !== 1 ? "s" : ""}{" "}
                  found
                </span>
              </div>
            </div>
                  ...
                  <div className="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex justify-center sm:justify-end">
                      <Pagination
                        ...
                        totalItem={totalAttendance || attendances?.length || 0}
                        perPage={perPage}
                        showItem={Math.min(
                          5,
                          Math.ceil(
