...
const ManagerEmployeeAttendance = () => {
  ...
  const [currentFetchingDepartment, setCurrentFetchingDepartment] =
    useState("");
  ...
  // Clear attendance data when employee changes
  useEffect(() => {
  ...
  const getAttendanceByDepartment = useCallback(() => {
    ...
    const requestId = `${selectedDepartment}-${dutySchedule._id}-${Date.now()}`;
  ...
  }, [dispatch, dutySchedule, selectedDepartment]);
  ...
  // Department change effect - simplified
  useEffect(() => {
    if (!selectedDepartment) {
      setOriginalAttendances([]);
      setFilteredAttendances([]);
      setTotalFilteredAttendance(0);
      setCurrentFetchingDepartment("");
      return;
    }

    if (selectedDepartment === currentFetchingDepartment) {
      return;
    }

    setCurrentFetchingDepartment(selectedDepartment);
  ...
  }, [dispatch, selectedDepartment, currentFetchingDepartment]);
  ...
  // Process attendance data - ONLY when not loading and data matches current request
  useEffect(() => {
    if (
      !attendanceLoading &&
      attendances &&
      selectedDepartment &&
      selectedDepartment === currentFetchingDepartment &&
      currentRequestId &&
      currentRequestId.startsWith(selectedDepartment) &&
      dutySchedule?._id &&
      currentRequestId.includes(dutySchedule._id)
    ) {
      console.log(
        `Processing ${attendances.length} attendance records for department ${selectedDepartment}`
      );

      const newAttendances = [...attendances];
      setOriginalAttendances(newAttendances);
      setFilteredAttendances(newAttendances);
      setTotalFilteredAttendance(newAttendances.length);
    }
  }, [
    attendances,
    attendanceLoading,
    selectedDepartment,
    currentFetchingDepartment,
    currentRequestId,
    dutySchedule,
  ]);
  ...
  const handleDepartmentChange = (e) => {
    ...
    } else if (departmentId !== selectedDepartment) {
      setCurrentRequestId(null);
      setOriginalAttendances([]);
      setFilteredAttendances([]);
      setTotalFilteredAttendance(0);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* ONLY show loading when attendanceLoading is true */}
      {attendanceLoading && departmentLoading ? (
        <LoadingSkeleton />
      ) : // Show table when we have data OR when we're ready to show "no records"
      (filteredAttendances && filteredAttendances.length > 0) ||
        (!attendanceLoading &&
          selectedDepartment &&
          currentFetchingDepartment === selectedDepartment) ? (
        <>
