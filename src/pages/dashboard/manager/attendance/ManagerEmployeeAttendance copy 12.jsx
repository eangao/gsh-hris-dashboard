...
const ManagerEmployeeAttendance = () => {
  ...
  // Modified loading logic - only show main loading on first load or department change
  const loading =
    managedDepartmentsLoading ||
    (!selectedDepartment && managedDepartments?.length > 0
      ? false
      : attendanceLoading || dutyScheduleLoading || employeesLoading);
  ...
  const getAttendanceByDepartment = useCallback(() => {
    ...
    // Only set loading page for initial load or department change, not for search/pagination
    if (isInitialLoad || isDepartmentChanging) {
  ...
  }, [dispatch, dutySchedule, isInitialLoad, isDepartmentChanging]);

  // Handle page changes - no loading since data is frontend filtered
  ...
  useEffect(() => {
    ...
    // Set department changing flag for loading state
  ...
  // Process and filter attendance data - no loading for search filtering
  useEffect(() => {
    if (originalAttendances && originalAttendances.length > 0) {
      // No loading state for search filtering since it's instant
      ...
      console.log("Should show pagination:", filteredData.length > perPage);

      setFilteredAttendances(filteredData);
      setTotalFilteredAttendance(filteredData.length);

      // Reset page to 1 if current page would be empty after filtering
  ...
  }, [searchValue, originalAttendances, employees, currentPage, perPage]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Loading indicator - only for department changes, not pagination */}
      <LoadingIndicator isLoading={isDepartmentChanging} />

      {/* Show loading skeleton while data is being fetched on initial load or department change */}
      {(loading || isLoadingPage) && (isInitialLoad || isDepartmentChanging) ? (
        <LoadingSkeleton />
      ) : (
        <>
          {/* Attendance Table */}
          <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200">
            ...
            {/* Pagination - show when filtered data exceeds perPage */}
            {console.log("Pagination check:", {
              filteredLength: filteredAttendances?.length,
              perPage,
              shouldShow:
            ...
            {filteredAttendances && filteredAttendances.length > perPage && (
              <div className="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between">
