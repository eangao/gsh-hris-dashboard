...
import { directorApproval, messageClear } from "../../../../store/Reducers/dutyScheduleReducer";
import toast from "react-hot-toast";

import { fetchManagedCluster } from "../../../../store/Reducers/employeeReducer";
import { fetchDutySchedulesByCluster } from "../../../../store/Reducers/dutyScheduleReducer";
...
const DirectorDutySchedule = () => {
  ...
  const [remarksModal, setRemarksModal] = useState({ open: false, scheduleId: null });
  const [remarks, setRemarks] = useState("");
  const { successMessage, errorMessage } = useSelector((state) => state.dutySchedule);
  ...
  }, [successMessage, errorMessage, dispatch, managedCluster, perPage, currentPage, searchValue]);
  ...
  const handleSubmitRejection = () => {
    ...
    dispatch(directorApproval({ id: remarksModal.scheduleId, action: "reject", remarks }));
  };
            ...
            {loading ? (
              ...
              dutySchedules?.map((schedule) => (
                <tr key={schedule?._id} className="border-t">
                  ...
                  <td className="p-3 capitalize">{schedule?.department?.name}</td>
      ...
      {remarksModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md">
            <h2 className="text-lg font-bold mb-2 text-red-600">Reject Duty Schedule</h2>
            <textarea
